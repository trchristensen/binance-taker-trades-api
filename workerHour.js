...
const saveData = (takerData, symbol, startTime, endTime) => {
  ...
  jsonCache.set("takerDataHour", TakerData);
};
...
const rangeBuyVsSell = (tradesArr, lowMidHighRanges) => {
  ...
  const thirtythirdPercentile = stats.percentile(
    lowMidHighRanges.quants,
    0.333
  );
  const standardDeviation = stats.stdev(lowMidHighRanges.quants);
  const mean = stats.stdev(lowMidHighRanges.quants);
  const biggestTrade =
    lowMidHighRanges.quants[lowMidHighRanges.quants.length - 1];

  const lowMarketTrades = tradesArr.filter(
    trade => trade.quantity <= thirtythirdPercentile
  );
  const midMarketTrades = tradesArr.filter(
    trade =>
      trade.quantity > thirtythirdPercentile &&
      trade.quantity <= sixtysixthPercentile
  );
  const highMarketTrades = tradesArr.filter(
    trade => trade.quantity > sixtysixthPercentile
  );
...
// this timeout function inside the loop does not actually work the way i wanted it to,
// it is making it faster, if anything. which might be good anyway.
const runAll = symbols => {
  symbols.forEach(symbol =>
    setTimeout(() => {
      start(symbol, 3600000); // hour
      // start(symbol, 86400000); //day
    }, 1000)
  );
};
